...
form.addEventListener('submit', async (e) => {
    ...
    try {
        const response = await fetch('http://localhost:3000/get-attendance', {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            if (data.events && data.events.length > 0) {
                ...
                htmlContent += `<p class="italic">No events attended yet.</p>`;
            }
            htmlContent += `</div>`;
            ...
            if (data.departmentWork && data.departmentWork.length > 0) {
                ...
                htmlContent += `<p class="italic">No department work recorded yet.</p>`;
            }
            htmlContent += `</div>`;
